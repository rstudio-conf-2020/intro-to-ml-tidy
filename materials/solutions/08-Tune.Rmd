...
Create a knn workflow that tunes over `neighbors`. Then use `fit_grid`, `cv_folds` and `k10_20` to find the best value of `neighbors`. Save the output of `fit_grid`.
...
knn_results <- 
  knn_twf %>%
    fit_grid(resamples = cv_folds, grid = k10_20) 
...
pca_results <-
  pca_twf %>% 
    fit_grid(resamples = cv_folds, grid = nc10_40)
